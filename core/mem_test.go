...
func TestBIOSRead(t *testing.T) {
	...
	// Byte read from BIOS
	got8 := m.Read8(0xFF0000)
	if got8 != 0 {
		t.Errorf("BIOS byte read at 0xFF0000 = 0x%02X, want 0x00", got8)
	}

	// Read byte from reset vector area
	got8 = m.Read8(0xFFFF00)
	if got8 != 0x10 {
		t.Errorf("BIOS byte at 0xFFFF00 = 0x%02X, want 0x10", got8)
	}

	// Word read (LE)
	got16 := m.Read16(0xFFFF00)
	if got16 != 0x0010 {
		t.Errorf("BIOS word at 0xFFFF00 = 0x%04X, want 0x0010", got16)
	}

	// Long read (LE)
	got32 := m.Read32(0xFFFF00)
	if got32 != 0x00FF0010 {
		t.Errorf("BIOS long at 0xFFFF00 = 0x%08X, want 0x00FF0010", got32)
	}
}

func TestBIOSReadOnly(t *testing.T) {
	m := makeTestMemory(t)
	m.Write8(0xFFFF00, 0xAA)
	got := m.Read8(0xFFFF00)
	if got != 0x10 {
...
func TestWorkRAM(t *testing.T) {
	...
	// Byte
	m.Write8(0x4000, 0xAB)
	if got := m.Read8(0x4000); got != 0xAB {
		t.Errorf("workRAM byte = 0x%02X, want 0xAB", got)
	}

	// Word (LE)
	m.Write16(0x4010, 0xBEEF)
	if got := m.Read16(0x4010); got != 0xBEEF {
		t.Errorf("workRAM word = 0x%04X, want 0xBEEF", got)
	}
	// Verify LE byte decomposition
	if got := m.Read8(0x4010); got != 0xEF {
		t.Errorf("workRAM word low byte = 0x%02X, want 0xEF", got)
	}
	if got := m.Read8(0x4011); got != 0xBE {
		t.Errorf("workRAM word high byte = 0x%02X, want 0xBE", got)
	}

	// Long (LE)
	m.Write32(0x4020, 0xDEADBEEF)
	if got := m.Read32(0x4020); got != 0xDEADBEEF {
...
func TestZ80RAM(t *testing.T) {
	...
	m.Write8(0x7000, 0x42)
	if got := m.Read8(0x7000); got != 0x42 {
		t.Errorf("z80RAM byte = 0x%02X, want 0x42", got)
	}

	m.Write16(0x7010, 0x1234)
	if got := m.Read16(0x7010); got != 0x1234 {
		t.Errorf("z80RAM word = 0x%04X, want 0x1234", got)
	}

	m.Write32(0x7020, 0xCAFEBABE)
	if got := m.Read32(0x7020); got != 0xCAFEBABE {
...
func TestK2GE(t *testing.T) {
	...
	m.Write8(0x8000, 0xC0)
	if got := m.Read8(0x8000); got != 0xC0 {
		t.Errorf("k2ge byte = 0x%02X, want 0xC0", got)
	}

	m.Write16(0x8032, 0xABCD)
	if got := m.Read16(0x8032); got != 0xABCD {
		t.Errorf("k2ge word = 0x%04X, want 0xABCD", got)
	}

	m.Write32(0x8020, 0x11223344)
	if got := m.Read32(0x8020); got != 0x11223344 {
...
func TestCustomIO(t *testing.T) {
	...
	m.Write8(0x81, 0xAA)
	if got := m.Read8(0x81); got != 0xAA {
		t.Errorf("customIO byte = 0x%02X, want 0xAA", got)
	}

	m.Write8(0xFF, 0xBB)
	if got := m.Read8(0xFF); got != 0xBB {
...
func TestADRegisters(t *testing.T) {
	...
	if got := m.Read8(0x60); got != 0x3F {
		t.Errorf("A/D low before conversion = 0x%02X, want 0x3F", got)
	}
	if got := m.Read8(0x61); got != 0x00 {
...
func TestInputMapping(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			m.SetInput(packed)
			if got := m.Read8(0xB0); got != tc.wantB0 {
...
func TestCartCS0(t *testing.T) {
	...
	// Byte read
	if got := m.Read8(0x200000); got != 0x00 {
		t.Errorf("cart CS0 byte at 0 = 0x%02X, want 0x00", got)
	}
	if got := m.Read8(0x200005); got != 0x05 {
		t.Errorf("cart CS0 byte at 5 = 0x%02X, want 0x05", got)
	}

	// Word read (LE)
	if got := m.Read16(0x200000); got != 0x0100 {
...
func TestCartCS1(t *testing.T) {
	...
	if got := m.Read8(0x800000); got != 0xAA {
		t.Errorf("cart CS1 byte = 0x%02X, want 0xAA", got)
	}
	if got := m.Read16(0x800000); got != 0xBBAA {
...
func TestResetClearsRAM(t *testing.T) {
	...
	m.Write8(0x4000, 0xAB)
	m.Write8(0x7000, 0xCD)
	m.Write8(0x8000, 0xEF)
	m.Write8(0x20, 0x55)
	m.Write8(0x80, 0xAA)

	m.Reset()

	if got := m.Read8(0x4000); got != 0 {
		t.Errorf("workRAM after reset = 0x%02X, want 0x00", got)
	}
	if got := m.Read8(0x7000); got != 0 {
...
func TestZ80Activation(t *testing.T) {
	...
	// Default is $AA (off)
	if got := m.Read8(0xB9); got != 0xAA {
		t.Errorf("z80Active default = 0x%02X, want 0xAA", got)
	}

	// Write $55 (on), read back
	m.Write8(0xB9, 0x55)
	if got := m.Read8(0xB9); got != 0x55 {
...
func TestT6W28WriteGate(t *testing.T) {
	...
	m.Write8(0xB8, 0x55)
	m.Write8(0xA0, 0x9F) // max vol ch0 right
	m.Write8(0xA1, 0x9F) // max vol ch0 left
	...
	if got := m.Read8(0xA0); got != 0x80 {
		t.Errorf("$A0 customIO = 0x%02X, want 0x80", got)
	}
	if got := m.Read8(0xA1); got != 0x80 {
...
func TestNMIEnable(t *testing.T) {
	...
	m.Write8(0xB3, 0x04) // bit 2 = power button NMI enable
	if got := m.Read8(0xB3); got != 0x04 {
		t.Errorf("NMI enable = 0x%02X, want 0x04", got)
	}

	m.Write8(0xB3, 0x00)
	if got := m.Read8(0xB3); got != 0x00 {
...
func TestRTSState(t *testing.T) {
	...
	m.Write8(0xB2, 0x01)
	if got := m.Read8(0xB2); got != 0x01 {
		t.Errorf("RTS state = 0x%02X, want 0x01", got)
	}

	m.Write8(0xB2, 0x00)
	if got := m.Read8(0xB2); got != 0x00 {
...
func TestDACRegisters(t *testing.T) {
	...
	if got := m.Read8(0xA2); got != 0x42 {
		t.Errorf("DAC L = 0x%02X, want 0x42", got)
	}
	if got := m.Read8(0xA3); got != 0xBE {
...
func TestDACReset(t *testing.T) {
	...
	if got := m.Read8(0xA2); got != 0x80 {
		t.Errorf("DAC L after reset = 0x%02X, want 0x80", got)
	}
	if got := m.Read8(0xA3); got != 0x80 {
...
func TestSetVBlankStatus(t *testing.T) {
	...
	if got := m.Read8(0x8010); got&0x40 != 0 {
...
func TestK2GEModeLock(t *testing.T) {
	...
	if got := m.Read8(0x87E2); got != 0x00 {
	...
	m.Write8(0x87E2, 0x80)
	if got := m.Read8(0x87E2); got != 0x80 {
	...
	if got := m.Read8(0x87F0); got != 0x00 {
...
func TestK2GESoftwareReset(t *testing.T) {
	...
	if got := m.Read8(0x87E0); got != 0x00 {
...
func TestK2GEWordWriteAcrossReadOnly(t *testing.T) {
	...
	if got := m.Read8(0x8011); got != 0xBE {
...
func TestHBlank(t *testing.T) {
	...
	m.Write8(0x22, 2)    // TREG0 = 2
	m.Write8(0x24, 0x00) // T01MOD: T0CLK=00(external)
	m.Write8(0x20, 0x81) // TRUN: T0 run + prescaler
...
func TestK2GEReadLEDRegisters(t *testing.T) {
	...
	m.Write8(0x8400, 0x00)
	if got := m.Read8(0x8400); got != 0x07 {
		t.Errorf("$8400 after write $00 = 0x%02X, want 0x07", got)
	}

	m.Write8(0x8400, 0xF8)
	if got := m.Read8(0x8400); got != 0xFF {
		t.Errorf("$8400 after write $F8 = 0x%02X, want 0xFF", got)
	}

	// LED_FLC ($8402): reset default is $80
	if got := m.Read8(0x8402); got != 0x80 {
...
func TestK2GEReadShadeLUTMask(t *testing.T) {
	...
	m.Write8(0x8100, 0xFF)
	if got := m.Read8(0x8100); got != 0x07 {
		t.Errorf("$8100 after write $FF = 0x%02X, want 0x07", got)
	}

	m.Write8(0x8117, 0xFF)
	if got := m.Read8(0x8117); got != 0x07 {
		t.Errorf("$8117 after write $FF = 0x%02X, want 0x07", got)
	}

	m.Write8(0x8108, 0xA5)
	if got := m.Read8(0x8108); got != 0x05 {
...
func TestK2GEReadUnmaskedPassthrough(t *testing.T) {
	...
	m.Write8(0x8020, 0xAB)
	if got := m.Read8(0x8020); got != 0xAB {
...
func TestK2GEReadControlMask(t *testing.T) {
	...
	m.Write8(0x8012, 0xFF)
	if got := m.Read8(0x8012); got != 0xCF {
		t.Errorf("$8012 after write $FF = 0x%02X, want 0xCF", got)
	}

	// Writing only truly unused bits (5-4) yields 0
	m.Write8(0x8012, 0x30)
	if got := m.Read8(0x8012); got != 0x00 {
...
func TestK2GEReadScrollPriorityMask(t *testing.T) {
	...
	m.Write8(0x8030, 0xFF)
	if got := m.Read8(0x8030); got != 0x80 {
		t.Errorf("$8030 after write $FF = 0x%02X, want 0x80", got)
	}

	m.Write8(0x8030, 0x7F)
	if got := m.Read8(0x8030); got != 0x00 {
...
func TestK2GEReadBGSelectionMask(t *testing.T) {
	...
	m.Write8(0x8118, 0xFF)
	if got := m.Read8(0x8118); got != 0xC7 {
		t.Errorf("$8118 after write $FF = 0x%02X, want 0xC7", got)
	}

	m.Write8(0x8118, 0x38)
	if got := m.Read8(0x8118); got != 0x00 {
...
func TestK2GEReadSpritePalAsnMask(t *testing.T) {
	...
	m.Write8(0x8C00, 0xFF)
	if got := m.Read8(0x8C00); got != 0x0F {
		t.Errorf("$8C00 after write $FF = 0x%02X, want 0x0F", got)
	}

	m.Write8(0x8C3F, 0xFF)
	if got := m.Read8(0x8C3F); got != 0x0F {
...
func TestK2GEPaletteWordAccess(t *testing.T) {
	...
	m.Write16(0x8200, 0x0FFF)
	if got := m.Read16(0x8200); got != 0x0FFF {
		t.Errorf("palette word at $8200 = 0x%04X, want 0x0FFF", got)
	}

	// Word write at end of palette range
	m.Write16(0x83FE, 0x0ABC)
	if got := m.Read16(0x83FE); got != 0x0ABC {
...
func TestK2GEPaletteByteAccess(t *testing.T) {
	...
	m.Write8(0x8200, 0xFF)

	// Byte read should return actual stored value
	if got := m.Read8(0x8200); got != 0xFF {
	...
	m.Write16(0x83FE, 0x0ABC)
	if got := m.Read8(0x83FF); got != 0x0A {
...
func TestK2GEResetLEDFLC(t *testing.T) {
	...
	// Overwrite LED_FLC
	m.Write8(0x8402, 0x00)
	if got := m.Read8(0x8402); got != 0x00 {
