...
func TestPinWordRead(t *testing.T) {
	...
	mem.Write16(0x4000, 0xCDAB)

	// Pin only the high byte ($4001).
	bus.Pin(0x4001, 0xFF)
	got := bus.Read16(0x4000)
	if got != 0xFFAB {
...
func TestMirrorByteRead(t *testing.T) {
	...
	// Write source value.
	mem.Write8(0x4010, 0x99)

	// Mirror $4020 -> $4010.
	bus.Mirror(0x4020, 0x4010)
	got := bus.Read8(0x4020)
	if got != 0x99 {
...
func TestMirrorWordRead(t *testing.T) {
	...
	mem.Write8(0x4010, 0x11)
	mem.Write8(0x4011, 0x22)

	// Mirror two bytes: $4020->$4010, $4021->$4011.
	bus.Mirror(0x4020, 0x4010)
	bus.Mirror(0x4021, 0x4011)

	got := bus.Read16(0x4020)
	if got != 0x2211 {
...
func TestNoOverlayPassthrough(t *testing.T) {
	...
	mem.Write32(0x4000, 0xDEADBEEF)
	got := bus.Read32(0x4000)
	if got != 0xDEADBEEF {
